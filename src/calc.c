...
/*   Updated: 2016/03/19 16:22:32 by pdelefos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	isometric(t_mlx *mlx, t_map *map, t_mov *mov)
{
	t_coord	o;
	t_coord b;
	int		tile;
	...
	while (o.y < map->lines)
	{
		...
		{
			map->print[o.y][o.x].x = (o.x - o.y) * tile + b.x + mov->dir.x;
			map->print[o.y][o.x].y = (o.x + o.y) * (tile / 2) + b.y -
				(map->parse[o.y][o.x] * mov->haut) + mov->dir.y;
			o.x++;
		}
		o.y++;
	}
